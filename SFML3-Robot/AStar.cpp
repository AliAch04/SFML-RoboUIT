...
// Constructeur de PathFinder 
PathFinder::PathFinder() {
    heuristic = std::make_unique<ManhattanHeuristic>();
}

// isSolvable reste dans PathFinder
...
// Implémentation de findPath pour AStar
std::vector<Point> AStar::findPath(Maze* maze) {
    explored.clear();
    ...
    struct PQComp {
        bool operator()(PQNode const& a, PQNode const& b) const {
            return a.f > b.f;
        }
    };
